...
def title_analysis(text: str) -> str:
    messages = [
        SystemMessage(
            content=(
                "你现在的任务是：只根据「整封邮件的日文标题（subject）」来判断这封邮件的类型，并输出一个固定整型标签。\n\n"
                "分类规则如下：\n"
                "・输出 0：表示「求人」——我这边有案件 / 项目 / 工作机会，希望寻找可以做这个案件的人、工程师或候选人。\n"
                "・输出 1：表示「求案件」——我这边有人 / 候选人 / 工程师 / 团队，希望寻找合适的案件 / 项目 / 职位 / 工作来对接。\n"
                "・输出 -1：表示其他类型——既不是在发布招聘 / 提供案件，也不是在主动寻找案件 / 工作，例如问候、通知、发票、系统邮件等。\n\n"
                "重要要求：\n"
                "1. 只看「标题」进行判断，不要臆测正文内容。\n"
...
if __name__ == "__main__":
    ...
    print(
        title_analysis(
            "※面談組めてます！情シス/社員代替案件！★【元請直 / インフラ / AWS / PMO / PM / PL / 社員代替 / 70～85万】@六本木一丁目駅 / 生保業向けAWSインフラ構築/保守案件の推進業務(社員代替)【サクヤ 鵜瀬】★"
        )
    )
